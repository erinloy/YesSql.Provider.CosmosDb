...
> **Status: working core, ~81% of YesSql's own conformance suite passing (202/249).**
> Document CRUD, map indexes, and the common query surface work end-to-end against the Cosmos
> emulator. Reduce indexes, multi-index joins, and transaction rollback are not yet implemented (some
> are bounded by Cosmos itself). See [`docs/CONFORMANCE.md`](docs/CONFORMANCE.md) for the full matrix
